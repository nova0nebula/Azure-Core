...
# Variables
azure_core_commands = {"Restart-System":"Restarts the system","Shutdown-System":"Shutdowns the system","help":"Show this help menu","version":"Display the current version of Azure Core","--version":"Display the current version of Azure Core","clear":"Clears the screen","list-modules":"Lists all the modules","changelog":"Displays the changelog","checkupdates":"Checks for updates","ls":"List files in the current directory","sysinfo":"Display system information","tasklist":"Show running processes","ipconfig":"Show network configuration"}
...
def azure_core_help_screen(prompt):
  ...
  for command,usage in azure_core_commands.items():
    ...
    print(f"{Fore.CYAN}ac{Fore.RESET} {command}"+(" "*length)+f"{Fore.GREEN}- {usage}{Fore.RESET}")
  ...
  if action in azure_core_commands:
    try:
      if action in ["version","--version"]:
        ...
        print(f"{Fore.MAGENTA}Platform: {system_name} {platform.release()}{Fore.RESET}")
...
def azure_core_main():
  ...
  while True:
    prompt = str(input(f"{Fore.CYAN}AC{Fore.RESET} C:\\{Fore.LIGHTYELLOW_EX}Users{Fore.RESET}\\{username}> "))
    parts = prompt.split()
    if not parts:
      continue
      ...
      if prefix == "ac":
        ...
        if action == "help":
          ...
          for key, value in prefixes.items():
            ...
            length = 25-len(key)
            print(f"{colour}{key}"+(" "*length)+f"{colour}- {value}{Fore.RESET}")
    else:
